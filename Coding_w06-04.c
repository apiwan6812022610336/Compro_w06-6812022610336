...
int main() {        
    ...
    // กำหนดตัวเเปร
    double y = 7.0, z = 5.0, a = 2.0;
    double price = 100.0, quantity = 3.0, discount = 10.0;
    double penalty = 5.0;
    int mistake = 2; 
    

    // --- การเปรียบเทียบที่ 1: x = x - 4.0;  เทียบกับ  x -= 4.0; ---
    ...
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (x_val_case1_long == x_val_case1_short) ? "YES" : "NO");


    // --- การเปรียบเทียบที่ 2: x = 6.5 * x;  เทียบกับ  x *= 6.5; ---
    printf("2. x = 6.5 * x; และ x *= 6.5;\n");
    double x_val_case2_long = 5.0;
    x_val_case2_long = 6.5 * x_val_case2_long;
    printf("   ผลลัพธ์ (รูปแบบเต็ม) : %.2f\n", x_val_case2_long);

    double x_val_case2_short = 5.0;
    x_val_case2_short *= 6.5;
    printf("   ผลลัพธ์ (รูปแบบย่อ)  : %.2f\n", x_val_case2_short);
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (x_val_case2_long == x_val_case2_short) ? "YES" : "NO");


    // --- การเปรียบเทียบที่ 3: x = x %% (y + z * a);  เทียบกับ  x %%= (y + z * a); ---
    ...
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (ix_val_case3_long == ix_val_case3_short) ? "YES" : "NO");


    // --- การเปรียบเทียบที่ 4: x = x / (2.0 * x);  เทียบกับ  x /= (2.0 * x); ---
    printf("4. x = x / (2.0 * x); และ x /= (2.0 * x);\n");
    double x_val_case4_long = 20.0;
    x_val_case4_long = x_val_case4_long / (2.0 * x_val_case4_long);
    printf("   ผลลัพธ์ (รูปแบบเต็ม) : %.2f\n", x_val_case4_long);

    double x_val_case4_short = 20.0;
    x_val_case4_short /= (2.0 * x_val_case4_short);
    printf("   ผลลัพธ์ (รูปแบบย่อ)  : %.2f\n", x_val_case4_short);
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (x_val_case4_long == x_val_case4_short) ? "YES" : "NO");
    ...
    double total_val_case5_long = 50.0;
    total_val_case5_long = total_val_case5_long + (price * quantity - discount);
    printf("   ผลลัพธ์ (รูปแบบเต็ม) : %.2f\n", total_val_case5_long);

    double total_val_case5_short = 50.0;
    total_val_case5_short += (price * quantity - discount);
    printf("   ผลลัพธ์ (รูปแบบย่อ)  : %.2f\n", total_val_case5_short);
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (total_val_case5_long == total_val_case5_short) ? "YES" : "NO");
    ...
    double x_val_case6_long = 100.0;
    x_val_case6_long = x_val_case6_long * (1 + rate / 100.0); // ใช้ 100.0 เพื่อให้แน่ใจว่าเป็น Floating-point division
    printf("   ผลลัพธ์ (รูปแบบเต็ม) : %.2f\n", x_val_case6_long);

    double x_val_case6_short = 100.0;
    x_val_case6_short *= (1 + rate / 100.0);
    printf("   ผลลัพธ์ (รูปแบบย่อ)  : %.2f\n", x_val_case6_short);
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (x_val_case6_long == x_val_case6_short) ? "YES" : "NO");
    ...
    double score_val_case7_long = 500.0;
    score_val_case7_long = score_val_case7_long - (penalty * (mistake + 1));
    printf("   ผลลัพธ์ (รูปแบบเต็ม) : %.2f\n", score_val_case7_long);

    double score_val_case7_short = 500.0;
    score_val_case7_short -= (penalty * (mistake + 1));
    printf("   ผลลัพธ์ (รูปแบบย่อ)  : %.2f\n", score_val_case7_short);
    printf("   ผลลัพธ์ตรงกัน: %s\n\n", (score_val_case7_long == score_val_case7_short) ? "YES" : "NO");
